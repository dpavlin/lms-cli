...
def main():
    p = argparse.ArgumentParser(description="LM Studio CLI Utility")
    s = p.add_subparsers(dest="cmd", help="Available commands")
    
    conf = s.add_parser("config", help="Configure the CLI settings")
    ...
    s.add_parser("status", help="Show server summary and loaded models")
    ...
    load.add_argument("--gpu", type=float, help="Set GPU offload ratio (0.0 to 1.0)")
    
    unl = s.add_parser("unload", help="Unload a model instance")
    unl.add_argument("model_id", help="The instance ID or model ID to unload")
    
    inf = s.add_parser("info", help="Get detailed model metadata")
    inf.add_argument("model_id", help="The ID of the model to inspect")
    
    src = s.add_parser("search", help="Search your local model library")
    src.add_argument("query", help="The search term")
    
    dl = s.add_parser("download", help="Download a model from Hugging Face")
    dl.add_argument("model_id", help="Hugging Face repo or Model Catalog ID")
    ...
    rw.add_argument("--data", help="JSON data for POST requests")
